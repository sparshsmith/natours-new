...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    if (!freshUser) next(new AppError('The user belonging to the user does not exists.', 401));

    // 4. User changed password after token was issued
    if (freshUser.changePasswordAfter(decoded.iat)) return next(new AppError('User recently changed password. Please login again!', 401));
...
exports.isLoggedIn = async (req, res, next) => {
    try {
        if (req.cookies.jwt) {
            ...
            // 3. User changed password after token was issued
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    if (!user) {
        return next(new AppError('There is no user associated with email address', 404));
    }

    // 2. Generate the random reset link
